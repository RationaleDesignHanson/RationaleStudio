...
export default function DemoSlidePage() {
  ...
  return (
    ...
    >
      {/* Demo Introduction */}
      <CRECard variant="accent" size="large" className="mb-8 text-center">
        <H2 className="mb-4">Interactive Platform Demo</H2>
        <BodyLG color={CRE_COLORS.text.secondary} className="max-w-3xl mx-auto">
          Experience CREaiT as if you were a broker. Click through the tabs below to see daily
          prioritization, transparent AI scoring, and context-aware email generation.
        </BodyLG>
      </CRECard>

      {/* Tab Navigation */}
      <div className="mb-8">
        <div className="flex gap-3 p-2 rounded-xl bg-white/5 border border-white/10">
          {tabs.map((tab) => (
            <button
              key={tab.id}
      ...
      <CRECard variant="neutral" size="large">
        <div className="min-h-[600px]">
          {activeTab === 'dashboard' && <OpportunityDashboardDemo />}
          {activeTab === 'scoring' && <ScoreBreakdownDemo />}
          {activeTab === 'emails' && <EmailDraftsDemo />}
        </div>
      </CRECard>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
        <CRECard variant="primary">
          <div className="flex items-start gap-3">
            ...
            <div>
              <H3 className="mb-2 text-lg">Real Data</H3>
              <BodyMD color={CRE_COLORS.text.secondary}>
                All examples use anonymized data from actual pilot customer pipelines
              </BodyMD>
            </div>
          </div>
        </CRECard>
        ...
        <CRECard variant="secondary">
          <div className="flex items-start gap-3">
            <span className="text-3xl">⚡</span>
            <div>
              <H3 className="mb-2 text-lg">Fully Interactive</H3>
              <BodyMD color={CRE_COLORS.text.secondary}>
                Sort, filter, and explore the interface exactly as brokers would
              </BodyMD>
            </div>
          </div>
        </CRECard>
        ...
        <CRECard variant="accent">
          <div className="flex items-start gap-3">
            <span className="text-3xl">🤖</span>
            <div>
              <H3 className="mb-2 text-lg">AI-Powered</H3>
              <BodyMD color={CRE_COLORS.text.secondary}>
                Scores and emails generated by Anthropic Claude with CRE-specific context
              </BodyMD>
            </div>
          </div>
        </CRECard>
