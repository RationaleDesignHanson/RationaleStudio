...
export default function WhyWeWinSlidePage() {
  ...
  return ( <CRESection
      ...
      variant="ask"
    > {/* Investment Thesis Header */} <CRECard variant="critical" size="large" className="mb-8 text-center"> <DisplayLG className="mb-6" color={CRE_COLORS.score.critical}> Investment Thesis </DisplayLG> <BodyLG color={CRE_COLORS.text.secondary} className="max-w-4xl mx-auto"> CREaiT solves a $9B problem with no direct competition, validated customer demand, and
          80% of the technical work already complete. This is a rare opportunity to invest in a
          capital-efficient, defensible B2B SaaS company at the perfect inflection point. </BodyLG> </CRECard> {/* Six Reasons */} <H2 className="mb-6">Six Reasons to Invest</H2> <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8"> {investmentThesis.map((reason, index) => ( <CRECard key={index} variant={index < 2 ? 'primary' : 'secondary'}> <div className="text-center mb-4"> <div className="text-5xl mb-3">{reason.icon}</div> <H3>{reason.title}</H3> </div> <ul className="space-y-2 pt-4 border-t border-white/10"> {reason.points.map((point, idx) => ( <li key={idx} className="flex items-start gap-2"> <span className="text-green-400 mt-1">→</span> <BodyMD color={CRE_COLORS.text.secondary}>{point}</BodyMD> </li> ))} </ul> </CRECard> ))} </div> {/* Founder Strengths */} <CRECard variant="accent" className="mb-8"> <H2 className="mb-6">Why This Founder Can Execute</H2> <div className="grid grid-cols-1 md:grid-cols-2 gap-6"> {founderStrengths.map((strength, index) => ( <div
              key={index}
              className="p-4 rounded-lg bg-white/5 border border-white/10"
            > <H3 className="text-lg mb-2">{strength.area}</H3> <BodyMD color={CRE_COLORS.text.secondary}>{strength.evidence}</BodyMD> </div> ))} </div> </CRECard> {/* Closing Statement */} <CRECard variant="primary"> <div className="flex items-start gap-4"> <div className="text-2xl md:text-3xl lg:text-4xl"></div> <div className="flex-1"> <H2 className="mb-4">The Opportunity</H2> <BodyLG color={CRE_COLORS.text.secondary} className="mb-4"> Commercial real estate is a $2.5 trillion market where brokers still rely on gut feel
              and spreadsheets. CREaiT brings AI-powered intelligence to an industry desperate for
              it — at exactly the right moment. </BodyLG> <BodyLG color={CRE_COLORS.text.secondary}> With backend 80% complete, 5 pilots committed, and a clear path to $300K ARR in 12
