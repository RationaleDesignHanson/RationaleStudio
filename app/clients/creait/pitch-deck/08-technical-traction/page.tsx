  ...
  return (
    ...
    >
      ...
      <CRECard variant="primary" size="large" className="mb-8 text-center">
        <LabelSM color={CRE_COLORS.text.muted}>INVESTED VALUE TO DATE</LabelSM>
        <DataXL className="my-6" color={CRE_COLORS.primary}>
          $94K
        </DataXL>
        <BodyLG color={CRE_COLORS.text.secondary}>
          Backend infrastructure complete. Production-ready. 14 weeks to MVP frontend.
        </BodyLG>
      </CRECard>

      {/* Completed Work */}
      <H2 className="mb-6">✅ What's Built (80% Complete)</H2>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
        {completedWork.map((item, index) => (
          <CRECard key={index} variant="accent">
            <div className="flex items-start justify-between mb-2">
              <H3 className="text-base">{item.component}</H3>
            ...
            <div className="pt-3 border-t border-white/10">
              <LabelSM color={CRE_COLORS.accent}>Invested Value: {item.value}</LabelSM>
            </div>
          </CRECard>
        ))}
      </div>
      ...
      {/* Remaining Work */}
      <H2 className="mb-6">⏳ What's Left (20% Remaining)</H2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        {remainingWork.map((item, index) => (
          <CRECard key={index} variant="neutral">
            <H3 className="text-lg mb-2">{item.component}</H3>
            <BodyMD color={CRE_COLORS.text.secondary} className="mb-3">
              {item.description}
            </BodyMD>
            <div className="pt-3 border-t border-white/10">
              <LabelSM color={CRE_COLORS.text.muted}>Effort: {item.effort}</LabelSM>
            </div>
          </CRECard>
        ))}
      </div>
      ...
      <CRECard variant="secondary">
        <H2 className="mb-6">Modern, Production-Ready Tech Stack</H2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {techStack.map((item, index) => (
            ...
            >
              <LabelSM color={CRE_COLORS.secondary} className="mb-2">
                {item.category}
              </LabelSM>
              <BodyMD>{item.tech}</BodyMD>
            </div>
          ))}
        </div>
      </CRECard>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
        <CRECard variant="primary">
          <div className="flex items-start gap-3">
            ...
            <div>
              <H3 className="mb-3">Speed to Market</H3>
              <BodyMD color={CRE_COLORS.text.secondary}>
                With 80% of backend complete, we can ship MVP in 14 weeks (not 6+ months). Reduces
                seed capital burn and accelerates pilot program timeline.
              </BodyMD>
            </div>
          </div>
        </CRECard>
        ...
        <CRECard variant="primary">
          <div className="flex items-start gap-3">
            ...
            <div>
              <H3 className="mb-3">De-Risked Investment</H3>
              <BodyMD color={CRE_COLORS.text.secondary}>
                $94K of founder sweat equity already invested. Production infrastructure deployed.
                You're investing in go-to-market, not unproven technical feasibility.
              </BodyMD>
            </div>
          </div>
        </CRECard>
