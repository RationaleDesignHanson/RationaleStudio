  ...
  return (
    <div className="w-full">
      ...
      <div className="mb-8 md:mb-12">
        <h3 className="mb-3 md:mb-6 text-xl md:text-2xl font-bold text-[#2D2D2D]">TestFlight Beta Metrics</h3>
        <p className="mb-4 md:mb-8 max-w-3xl text-xs md:text-sm text-gray-600">
        ...
        <div className="md:hidden flex gap-3 overflow-x-auto snap-x snap-mandatory pb-4 -mx-6 px-6">
          {testFlightMetrics.map((metric, idx) => (
            <div
              key={idx}
              className="flex-shrink-0 w-[75vw] snap-center group rounded-xl border-2 border-gray-200 bg-white p-4"
            >
              ...
              <div className="mb-2">
                <span className="text-2xl font-bold text-[#E85D4D]">{metric.value}</span>
                <span className="ml-1 text-base text-gray-500">{metric.unit}</span>
              </div>
        ...
        <div className="hidden md:grid gap-6 md:grid-cols-4">
          {testFlightMetrics.map((metric, idx) => (
            <div
              key={idx}
      ...
      {/* Performance Benchmarks */}
      <div className="mb-8 md:mb-12">
        <h3 className="mb-3 md:mb-6 text-xl md:text-2xl font-bold text-[#2D2D2D]">Performance Benchmarks</h3>
        <p className="mb-4 md:mb-8 max-w-3xl text-xs md:text-sm text-gray-600">
        ...
        <div className="md:hidden flex gap-3 overflow-x-auto snap-x snap-mandatory pb-4 -mx-6 px-6">
          {performanceMetrics.map((metric, idx) => (
            <div key={idx} className="flex-shrink-0 w-[70vw] snap-center rounded-xl border-2 border-gray-200 bg-white p-4">
              <div className="mb-3 text-xs font-semibold uppercase tracking-wide text-gray-500">{metric.label}</div>

              {/* Simplified mobile view - no circular progress */}
              <div className="mb-3">
                <span className="text-2xl font-bold text-[#E85D4D]">{metric.value}{metric.unit}</span>
              </div>
              ...
              {((metric.better === 'lower' && metric.value < metric.target) ||
                ...
                <div className="text-xs font-semibold text-[#2A9D8F]">
                  ✓ {metric.better === 'lower'
        ...
        {/* Desktop: Original grid with circular progress */}
        <div className="hidden md:grid gap-6 md:grid-cols-4">
          {performanceMetrics.map((metric, idx) => (
      ...
      {/* User Feedback */}
      <div className="mb-8 md:mb-12">
        <h3 className="mb-3 md:mb-6 text-xl md:text-2xl font-bold text-[#2D2D2D]">Beta Tester Feedback</h3>
        <p className="mb-4 md:mb-8 max-w-3xl text-xs md:text-sm text-gray-600">
        ...
        <div className="md:hidden flex gap-3 overflow-x-auto snap-x snap-mandatory pb-4 -mx-6 px-6">
          {feedbackHighlights.map((feedback, idx) => (
            <div key={idx} className="flex-shrink-0 w-[85vw] snap-center rounded-xl border-2 border-gray-200 bg-[#F5F1E8] p-4">
              ...
              <p className="mb-3 text-sm italic text-gray-700">"{feedback.quote}"</p>
      ...
      {/* Post-Launch Targets */}
      <div className="mb-8 md:mb-12">
        <h3 className="mb-3 md:mb-6 text-xl md:text-2xl font-bold text-[#2D2D2D]">Post-Launch Targets</h3>
        <p className="mb-4 md:mb-8 max-w-3xl text-xs md:text-sm text-gray-600">
        ...
        <div className="md:hidden flex gap-3 overflow-x-auto snap-x snap-mandatory pb-4 -mx-6 px-6">
          {postLaunchTargets.map((target, idx) => (
            <div key={idx} className="flex-shrink-0 w-[70vw] snap-center rounded-xl border-2 border-dashed border-gray-300 bg-white p-4">
              <div className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-500">{target.label}</div>
              <div className="mb-3 text-2xl font-bold text-[#E85D4D]">{target.target}</div>
