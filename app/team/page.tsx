...
export default function TeamDashboardPage() {
  ...
  return ( <div className="min-h-screen bg-gradient-to-b from-gray-900 via-black to-gray-900 text-white"> {/* Welcome Section */} <section className="py-12 px-4 sm:px-6 lg:px-8 border-b border-gray-800"> <div className="max-w-7xl mx-auto"> <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white mb-4"> {dashboard.welcome.title} </h1> <p className="text-xl text-purple-400 mb-4">{dashboard.welcome.subtitle}</p> <p className="text-base text-gray-300 max-w-3xl">{dashboard.welcome.description}</p> </div> </section> {/* Quick Stats */} <section className="py-12 px-4 sm:px-6 lg:px-8 border-b border-gray-800"> <div className="max-w-7xl mx-auto"> <h2 className="text-2xl font-bold text-white mb-8">System Overview</h2> <div className="grid grid-cols-2 md:grid-cols-4 gap-6"> {dashboard.quickStats.map((stat, idx) => ( <div
                key={idx}
                className="p-6 bg-gray-900/70 border border-gray-800 rounded-lg hover:border-purple-400/30 transition-colors"
              > <div className="text-3xl mb-3">{stat.icon}</div> <div className="text-3xl font-bold text-purple-400 mb-2">{stat.value}</div> <div className="text-sm font-semibold text-white mb-1">{stat.label}</div> <div className="text-xs text-gray-400">{stat.subtext}</div> {stat.trend && ( <div className="text-xs text-gray-500 mt-2"> <TrendingUp className="w-3 h-3 inline mr-1" /> {stat.trend} </div> )} </div> ))} </div> </div> </section> {/* Active Projects */} <section className="py-12 px-4 sm:px-6 lg:px-8 border-b border-gray-800"> <div className="max-w-7xl mx-auto"> <div className="flex items-center justify-between mb-8"> <h2 className="text-2xl font-bold text-white">Active Projects</h2> <Link
              href="/team/projects"
            ...
            > View All Projects <ArrowRight className="w-4 h-4" /> </Link> </div> <div className="space-y-6"> {dashboard.activeProjects.map((project, idx) => ( <div
              ...
              > <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 mb-4"> <div className="flex-1"> <div className="flex items-center gap-3 mb-2"> <h3 className="text-xl font-bold text-white">{project.name}</h3> <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                      ...
                      > {member} </span> ))} </div> </div> </div> ))} </div> </div> </section> {/* Recent Activity */} <section className="py-12 px-4 sm:px-6 lg:px-8 border-b border-gray-800"> <div className="max-w-7xl mx-auto"> <h2 className="text-2xl font-bold text-white mb-8">Recent Activity</h2> <div className="space-y-3"> {dashboard.recentActivity.map((activity, idx) => ( <div
                key={idx}
