...
export default function InvestorsPage() {
  ...
  return ( <> {/* Hero */} <Section spacing="large" background="transparent" colorTheme={heroTheme}> <ASCIIUnifiedGrid animated={true} colorTheme={heroTheme} opacity={0.25} /> <Container className="relative z-20"> <div className="max-w-6xl mx-auto"> <GlassCard theme={heroTheme} className="p-6 sm:p-8 lg:p-12" borderRadius="1.5rem"> <div className="text-center"> <div className="mb-4"> <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold" style={{ color: heroTheme.foreground }}> {overview.hero.title} </h1> </div> <p className="text-lg sm:text-xl mb-4" style={{ color: heroTheme.accent }}>{overview.hero.subtitle}</p> <p className="text-sm sm:text-base" style={{ color: heroTheme.muted }}>{overview.hero.description}</p> </div> </GlassCard> </div> </Container> </Section> {/* Dual-Engine Model */} <Section spacing="large" background="transparent" colorTheme={tiersTheme} noPaddingTop={true}> <ASCIIUnifiedGrid opacity={0.3} className="absolute top-0 left-0 right-0 z-10" colorTheme={tiersTheme} /> <ASCIIUnifiedGrid animated={true} colorTheme={tiersTheme} opacity={0.25} /> <Container className="relative z-20 pt-16 sm:pt-20"> <div className="max-w-6xl mx-auto"> <GlassCard theme={tiersTheme} className="p-6 sm:p-8 lg:p-12" borderRadius="1.5rem"> <div className="text-center mb-8"> <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-3" style={{ color: tiersTheme.foreground }}> {overview.model.title} </h2> <p className="text-lg mb-4" style={{ color: tiersTheme.accent }}>{overview.model.subtitle}</p> <p className="text-sm" style={{ color: tiersTheme.muted }}>{overview.model.description}</p> </div> <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6"> {overview.model.metrics.map((metric, idx) => ( <div
                    key={idx}
                  ...
                  > <div className="text-2xl sm:text-3xl md:text-4xl font-bold mb-2" style={{ color: tiersTheme.accent }}> {metric.value} </div> <div className="text-sm font-semibold mb-1" style={{ color: tiersTheme.foreground }}> {metric.label} </div> <div className="text-xs" style={{ color: tiersTheme.muted }}> {metric.subtext} </div> </div> ))} </div>
